...
export class AdherentService {
  ...
  getAdherentById() {
    return this.http.get(
      environment.APIUri + '/adherents/getAdherentById/' + this.adhId
    );
  }
  getAdherentById1(x): Observable<any> {
    return this.http.get<any>(
      environment.APIUri + '/adherents/getAdherentById/' + x
    );
  }
  getAdherentById2(x) {
    return this.http.get(
  ...
  getCredsAdherent(id) {
    ...
    this.getAdherentById1(id).subscribe((res) => {
      adherentCreds = JSON.stringify(res.nom_fr);

      console.log(res);
    });
  }
  desactiverAdherent(id){
  return this.http.put(environment.APIUri+'/etat_adherent/'+id,{
    etat_adherentId:id,


      etat_adherentLib: "inactif"
